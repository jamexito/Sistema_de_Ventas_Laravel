...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $totales = DB::select('SELECT (select ifnull(sum(c.total),0) from compras c where c.estado="Registrado" and MONTH (c.fecha_compra)=MONTH(CURRENT_DATE)) as totalcompra, (select ifnull(sum(v.total),0) from ventas v where MONTH(v.fecha_venta)=MONTH(CURRENT_DATE) and v.estado="Registrado") as totalventa');
